<template>
   ...
   <CCol :xs="12">
      ...
      <CCard class="mb-4">
         <CCardHeader>
            <strong>Lista de Rotas</strong>
            <small> - permite administrar as Rotas das Viagens</small>
         </CCardHeader>
         <CCardBody>
   ...
   <CModal
      ...
      @close="closeModal"
      backdrop="static"
      v-close-on-esc="closeModal"
   >
      <CModalHeader>
         <CModalTitle>{{
            isEditing
               ? (canUpdate ? 'Editar' : 'Ver') + ' Veículo'
               : 'Novo Veículo'
         }}</CModalTitle>
      </CModalHeader>
      <CModalBody>
         <CForm>
            <CFormInput
               ...
               label="Descrição"
               class="form-control"
               :class="{ 'is-invalid': fieldErrors.descricao }"
            />
            <div class="form-error" v-if="fieldErrors.descricao">
            ...
            <CFormSelect
               ...
               label="Tipo"
            />
            <div class="form-error" v-if="fieldErrors.tipo">
               {{ fieldErrors.tipo }}
            </div>

            <CFormInput v-model="form.marca_modelo" label="Marca/Modelo" />
            <div class="form-error" v-if="fieldErrors.marca_modelo">
               {{ fieldErrors.marca_modelo }}
            </div>

            <CFormInput v-model="form.capacidade" label="Capacidade" />
            <div class="form-error" v-if="fieldErrors.capacidade">
               {{ fieldErrors.capacidade }}
            </div>

            <CFormInput v-model="form.motorista" label="Motorista" />
            <div class="form-error" v-if="fieldErrors.motorista">
               {{ fieldErrors.motorista }}
            </div>

            <CFormInput v-model="form.telefone" label="Telefone" />
            <div class="form-error" v-if="fieldErrors.telefone">
      ...
      <CModalFooter>
         <div class="d-flex w-100 align-items-center justify-content-end">
            <div class="text-danger small mt-2 me-auto" v-if="formError">
               {{ formError }}
            </div>
            <div>
               <CButton
               ...
               <CButton
                  ...
                  @click="confirmSave()"
                  >{{ isEditing ? 'Salvar' : 'Criar' }}</CButton
               >
            </div>
         </div>
      </CModalFooter>
   ...
   >
      ...
      <CModalBody>
         <p>
            Tem certeza que deseja excluir este Registro: <br /><strong>{{
      ...
      <CModalFooter>
         <CButton class="btn btn-secondary btn-sm me-1" @click="cancelDelete"
            >Cancelar</CButton
         >
         <CButton class="btn btn-danger btn-sm" @click="confirmDelete"
            >Excluir</CButton
         >
      </CModalFooter>
   </CModal>
...
const { fetchVeiculos, canInsert, canUpdate, canDelete, canShow } =
   useRotasApi();
