<template>
  <CCol :xs="12">
    ...
    <CModal :visible="editModalVisible" @close="closeModal" backdrop="static" keyboard="true">
      ...
      <CModalBody>
        <CForm>
          <CFormInput v-model="form.descricao" label="Descrição" :invalid="!!fieldErrors.descricao" />
          <div class="text-danger small" v-if="fieldErrors.descricao">{{ fieldErrors.descricao }}</div>

          <CFormInput v-model="form.tipo" label="Tipo" :invalid="!!fieldErrors.tipo" />
          <div class="text-danger small" v-if="fieldErrors.tipo">{{ fieldErrors.tipo }}</div>
          ...
          <CFormSelect v-model="form.ativo" :options="[{ value: 'Y', label: 'SIM' }, { value: 'N', label: 'NÃO' }]"
            label="Ativo" />
        </CForm>
      </CModalBody>
      <CModalFooter>
        <CButton color="secondary" size="sm" @click="closeModal">Cancelar</CButton>
        <CButton :disabled="loading" color="primary" size="sm" @click="submit">Salvar</CButton>
      </CModalFooter>
    </CModal>
  </CCol>
  ...
  <CModal :visible="deleteModalVisible" @close="cancelDelete" backdrop="static" keyboard="true">
    ...
    <CModalBody>
      Tem certeza que deseja excluir este Registro: <br /><b>{{ selectedToDelete?.descricao }}</b> ?
    </CModalBody>
    <CModalFooter>
      <CButton color="btn btn-secondary btn-sm me-1" @click="cancelDelete">Cancelar</CButton>
      <CButton color="btn btn-danger    btn-sm me-1" @click="confirmDelete">Excluir</CButton>
    </CModalFooter>
  </CModal>
